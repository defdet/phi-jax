...
@partial(jax.jit, static_argnames=('logits_processor',))
def _generate_rest(params: Phi, seq: Array, attn_mask: Array, selected_token_ids: Array, max_n_iters: Array, logits_processor: Callable, *, rotary_values: RotaryValues, kv_cache: KVCache, key: Array) -> Array:
...
def generate(sentences: list[str], tokenizer: LlamaTokenizer, params: Phi, logits_processor: Callable, *, max_len: int, key: Array) -> list[str]:
    ...
    rotary_values = make_rotary_values(leftpad_len, batch_size, max_len, model_config=phi_config)
